...
let tableSortDir = 1;
let focusStock = '';  // From ?stock= param — lock view to this stock, then scroll+highlight it
let stockQuery = '';  // In-page stock query (code/name)
...
function saveFiltersToURL() {
    ...
    if (stockQuery) url.searchParams.set('stock_q', stockQuery);
    else url.searchParams.delete('stock_q');
...
function updateStockFilterUI() {
    const $input = $('#stock-filter-input');
    if (!$input.length) return;

    if ($input.val() !== stockQuery) $input.val(stockQuery);

    const isActive = !!(focusStock || stockQuery);
    $input.toggleClass('is-active', isActive);
    $('#stock-filter-clear').toggle(isActive);
}

function applyStockFilterFromInput(rawValue) {
    const nextQuery = normalizeStockQuery(rawValue);
    const hadFocusStock = !!focusStock;
    stockQuery = nextQuery;

    // If query changes away from URL-focused stock, release hard lock.
    if (hadFocusStock && stockQuery !== focusStock) focusStock = '';
    if (!stockQuery) focusStock = '';
...
$(document).ready(async function () {
    ...
    focusStock = normalizeStockQuery(params.get('stock') || '');
    stockQuery = normalizeStockQuery(params.get('stock_q') || '');
    if (focusStock && !stockQuery) stockQuery = focusStock;
    ...
    // Stock query filter (code/name)
    let stockFilterTimer = null;
    $('#stock-filter-input').on('input', function () {
        const value = $(this).val();
        clearTimeout(stockFilterTimer);
        stockFilterTimer = setTimeout(() => applyStockFilterFromInput(value), 180);
    });
    $('#stock-filter-input').on('keydown', function (e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            clearTimeout(stockFilterTimer);
            applyStockFilterFromInput($(this).val());
        }
    });
    $('#stock-filter-clear').on('click', function () {
        if (!stockQuery && !focusStock) return;
        stockQuery = '';
        focusStock = '';
        updateStockFilterUI();
        updateFilterButtons();
...
function updateFilterButtons() {
    ...
    const hasStockFilter = !!(focusStock || stockQuery);
    $('#btn-filter-reset').toggle(hasActive || hasStockFilter);
}
...
function applyStockScope(stocks) {
    const q = normalizeStockQuery(stockQuery);
    if (!focusStock && !q) return stocks;

    return stocks.filter(s => {
        const code = (s.stock_code || '').toUpperCase();
        const name = (s.stock_name || s.company_name || s.name || '').toUpperCase();

        if (focusStock && code !== focusStock) return false;
        if (!q) return true;
...
function renderSummary() {
    ...
    const scopedSummary = applyStockScope(summary);
    let netVal = 0, buyVal = 0, sellVal = 0;
    for (const s of scopedSummary) {
        netVal += s.total_net;
        buyVal += s.total_buy;
    ...
    $('#stat-breadth').text(scopedSummary.length);
...
function getTopStocks(n) {
    ...
    const focused = applyStockScope(enriched);
    ...
    const filtered = applyFilters(focused);
